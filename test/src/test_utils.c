    ...
    for (int c = 0; c < cases_num; ++c) {
        ...
        if (!teststates[c])
            ...
            if (shd_flag) {
                ...
                if (teststates[c]->shd_A_ref)
                    teststates[c]->shd_A_ref->shd_data = malloc(
                            test_matrices[c][0].rows * sizeof(shd_node *));

                if (teststates[c]->shd_B_ref)
                    teststates[c]->shd_B_ref->shd_data = malloc(
                            test_matrices[c][1].rows * sizeof(shd_node *));
